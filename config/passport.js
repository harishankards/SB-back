 ...
 */
passport.use('company-local', new LocalStrategy({ usernameField: 'email' }, (email, password, done) => {
...
passport.use('facebook-student', new FacebookStrategy({
  ...
  if (req.user) {
    Student.findOne({ facebook: profile.id }, (err, existingUser) => {
      if (err) { return done(err); }
      if (existingUser) {
    ...
    Student.findOne({ facebook: profile.id }, (err, existingUser) => {
      ...
      Student.findOne({ email: profile._json.email }, (err, existingEmailUser) => {
        if (err) { return done(err); }
...
passport.use('facebook-company', new FacebookStrategy({
  clientID: process.env.FACEBOOK_ID,
  clientSecret: process.env.FACEBOOK_SECRET,
  callbackURL: 'https://79741908.ngrok.io/auth/facebook/callback',
  profileFields: ['name', 'email', 'link', 'locale', 'timezone', 'gender'],
...
passport.use('company-linkedin', new LinkedInStrategy({
  ...
  if (req.user) {
    Company.findOne({ linkedin: profile.id }, (err, existingUser) => {
      if (err) { return done(err); }
      if (existingUser) {
        req.flash('errors', { msg: 'There is already a LinkedIn account that belongs to you. Sign in with that account or delete it, then link it with your current account.' });
        done(err);
    ...
    Company.findOne({ linkedin: profile.id }, (err, existingUser) => {
      ...
      Company.findOne({ email: profile._json.emailAddress }, (err, existingEmailUser) => {
        if (err) { return done(err); }
        if (existingEmailUser) {
          req.flash('errors', { msg: 'There is already an account using this email address. Sign in to that account and link it with LinkedIn manually from Account Settings.' });
          done(err);
...
exports.isAuthenticated = (req, res, next) => {
  console.log('inside isAuthenticated', req.body)
